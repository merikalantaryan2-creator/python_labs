...
def cat_command(
    input_path: str, numbered: bool = False
):  # нумерация. булл имеет только два значеия t и F
    ...
    with path.open(encoding="utf-8") as f:
        for i, line in enumerate(f, start=1):
            if numbered:
                print(
                    f"{i:4d}: {line.rstrip()}"
                )  # rstrip() убирает символы перевода строки
            else:
...
def stats_command(input_path: str, top_n: int = 5):
    ...
    with path.open(encoding="utf-8") as f:
        text = (
            f.read().lower()
        )  # читает как одну строку, переводит текст в нижний регистр

    words = [
        w.strip(".,!?;:\"'()[]") for w in text.split()
    ]  # удаляет знаки препинания с начала и конца каждого слова
    ...
    for w in words:
        ...
        freq[w] = (
            freq.get(w, 0) + 1
        )  #  получает значение по ключу w, если нет - возвращает 0
    ...
    for word, count in sorted_items:
        print(
            f"{word:15s} {count}"
        )  #  форматирование: строка word занимает 15 символов


def main():
    parser = argparse.ArgumentParser(
        description="CLI-утилиты для анализа текста (cat, stats)"
    )  # создает объект-парсер для обработки командной строки
    ...
    cat_parser.add_argument(
        "--input", required=True, help="Путь к файлу"
    )  #  аргумент ОБЯЗАТЕЛЕН для команды cat , инпут - имя аргумента
    cat_parser.add_argument("-n", action="store_true", help="Нумеровать строки")

    # ---- stats ----
    stats_parser = subparsers.add_parser("stats", help="Частоты слов")
    stats_parser.add_argument(
        "--input", required=True, help="Путь к текстовому файлу"
    )  # при наличии флага устанавливает значение True
