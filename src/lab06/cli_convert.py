...
def main():
    ...
    )  # создает подпарсеры для подкоманд
    ...
    # ---- json2csv ----
    p1 = sub.add_parser(
        "json2csv", help="Преобразовать JSON в CSV"
    )  # создает парсер для подкоманды
    ...
    if (
        args.cmd == "json2csv"
    ):  # это атрибут объекта args, который содержит название выбранной подкоманды.
